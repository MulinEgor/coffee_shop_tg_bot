...
class PositionService(
    ...
    async def create(
        self, data: PositionCreateSchema, include_related: bool = True
    ) -> PositionGetSchema:
        """
        ...
        if await self._repository.get_by_name(data.name, False):
            self._handle_error(
                "Позиция с таким названием уже существует", status_code=409
            )
    ...
    async def update(
        self, id: int, data: PositionUpdateSchema, include_related: bool = True
    ) -> PositionGetSchema:
        """
        ...
        if data.category_id and not await self._category_service.get(
