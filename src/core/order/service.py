...
from src.core.order.schemas import (
    OrderCreateSchema,
    OrderGetSchema,
...
class OrderService(
    Service[
        Order, OrderCreateSchema, OrderGetSchema, OrderUpdateSchema, OrderRepository
    ]
):
    """
    Сервис для заказов.
    ...
    async def create(
        self, data: OrderCreateSchema, include_related: bool = True
    ) -> OrderGetSchema:
        """
        Создание объекта.
    ...
    async def update(
        self, id: int, data: OrderUpdateSchema, include_related: bool = True
    ) -> OrderGetSchema:
        """
        Обновление объекта.
        ...
        # Проверка на наличие пользователя в бд
        if data.user_id is not None and not await self._user_repository.get(
            data.user_id, False
        ):
            self._handle_error("Пользователь с таким ID не найден", status_code=404)
    ...
    def _convert_to_schema(self, obj: Order) -> OrderGetSchema:
        ...
        try:
            order_positions = [
                OrderPositionGetSchema(
                    ...
                    position=self._position_service._convert_to_schema(
                        position.position
                    ),
                )
        ...
        ):  # Если объект не загружен, то возвращаем пустой список
            order_positions = []
            total_price = None
