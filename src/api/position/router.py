...
@router.get("/{id}", response_model=PositionGetSchema, status_code=status.HTTP_200_OK)
async def get(
...
@router.get("/", response_model=List[PositionGetSchema], status_code=status.HTTP_200_OK)
async def get_all(
...
@router.post("/", response_model=PositionGetSchema, status_code=status.HTTP_201_CREATED)
async def create(
...
@router.put("/{id}", response_model=PositionGetSchema, status_code=status.HTTP_200_OK)
async def update(
    id: int,
