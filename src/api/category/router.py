...
@router.get("/{id}", response_model=CategoryGetSchema, status_code=status.HTTP_200_OK)
async def get(
...
@router.get("/", response_model=List[CategoryGetSchema], status_code=status.HTTP_200_OK)
async def get_all(
...
@router.post("/", response_model=CategoryGetSchema, status_code=status.HTTP_201_CREATED)
async def create(
