...
async def category_callback(callback: CallbackQuery, state: FSMContext, position_service: PositionService):
    ...
    positions = [p for p in positions if p.category_id == category_id]
...
async def quantity_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_categories_keyboard(await CategoryService(None).get_all())
    )
...
async def obtaining_method_callback(callback: CallbackQuery, state: FSMContext, order_service: OrderService):
    ...
    order = await order_service.create(OrderCreateSchema(
        ...
        order_positions=[
            {"position_id": item.position_id, "quantity": item.quantity}
