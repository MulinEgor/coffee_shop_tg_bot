...
def get_role_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.add(
        InlineKeyboardButton(text="Бариста", callback_data=f"role:{Role.BARISTA.value}"),
...
def get_quantity_keyboard(position_id: int) -> InlineKeyboardMarkup:
    ...
    for i in range(1, 6):
        builder.add(InlineKeyboardButton(
            text=str(i),
            callback_data=f"quantity:{position_id}:{i}"
        ))
    builder.add(InlineKeyboardButton(
        text="◀️ Назад",
        callback_data=f"back_to_positions"
    ))
    builder.adjust(5, 1)
    return builder.as_markup()
...
def get_order_status_keyboard(order_id: int) -> InlineKeyboardMarkup:
    ...
    builder.add(
        InlineKeyboardButton(
            text="✅ Готов",
            callback_data=f"status:{order_id}:{Status.COMPLETED.value}"
        ),
        InlineKeyboardButton(
            text="❌ Отменён",
